    ...
    const crawler = new PlaywrightCrawler({
      requestHandler: async ({ request, page }) => {
        ...
        await page.waitForFunction(() =>
          document.querySelector('ul._list_181lzn > li')
        );
        const main_package = await page.$$eval('main', (package_lib) => {
          return package_lib.map((package_el) => {
            const packageName = package_el.querySelector(
              'h1._heading_8qtlic > span'
            )?.textContent;
            const keywords = Array.from(
              package_el.querySelectorAll('ul._keywords_8qtlic > li')
            ).map((keyList) => keyList?.textContent?.trim());
            const toText = (element: HTMLElement) =>
              element && element.textContent?.trim();
            const toLink = (element: HTMLElement) =>
              element && element.getAttribute('src');
            const timeUploaded = package_el
              ?.querySelector('time._date_rj9k29')
              ?.getAttribute('datetime');
            const license_name = package_el?.querySelector(
              'div._license_rj9k29 > span > a'
            )?.textContent;
            const license_url = package_el
              ?.querySelector('div._license_rj9k29 > span > a')
              ?.getAttribute('href');
            const version = package_el?.querySelector('small')?.innerText;
            const description = package_el
              ?.querySelector('div._description_8qtlic')
              ?.textContent?.trim();
            const totalDownloads = package_el
              .querySelector('span._num__align_87huyj')
              ?.textContent?.trim();
            const license = { type: license_name?.trim(), url: license_url };
            const links = Array.from(
              package_el.querySelectorAll('div._links_rj9k29 > div')
            ).map((link) => {
              return {
                name: link.querySelector('div > h2._heading_rj9k29')?.innerHTML,
                URL: link
                  .querySelector('div._content_t2rnmm > a._link_t2rnmm')
                  ?.getAttribute('href'),
              };
            });
            const owners = Array.from(
              package_el?.querySelectorAll('ul._list_181lzn > li')
            ).map((owners) => {
              const name = owners
            ...
            const webURL = `https://crates.io`;
            const installation = `cargo add ${packageName}`;
            const packageManager = 'cargo';
            const package_obj = {
              keywords,
              packageName,
              timeUploaded,
    ...
    requestHandler: async ({ page }) => {
      ...
      const main_article = await page.$$eval('html', (article) => {
        return article.map((art) => {
          const default_description = art
            .querySelector('meta[name="description"]')
            ?.getAttribute('content');
          const og_description = art
            .querySelector('meta[property="og:description"]')
            ?.getAttribute('content');
          const favicon = art
            .querySelector('link[rel="icon"]')
            ?.getAttribute('href');
          const article_link = art
